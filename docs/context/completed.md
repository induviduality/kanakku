...
- **#7/#8 Split period-scoping**: per explicit user-specified semantics — unsettled splits must appear in every period's view (not just the expense's own month) until settled; a settled multi-expense bundle split must appear in every month it has an expense transaction in, not just the earliest. Backend: `SplitResponse` gained `expense_dates: list[datetime]` (every linked expense's own date, not just `min()`). Frontend: `Splits.tsx` and `SplitsAll.tsx` (the `/splits/pending` and `/splits/history` view-all pages, fixing #7's `created_at`-filtering bug too) now compute visibility as "unsettled → always visible; settled → visible in any period matching any expense_date".
- **#9 Idle auto-logout + AuthGuard dead code**: `AuthGuard` (built to react to a mid-session token clear, never mounted) is now wrapped around all authenticated routes in `AppLayout.tsx`. New `useIdleLogout()` hook signs the user out after 20 minutes of no mouse/keyboard/touch/scroll activity, clearing local auth state immediately and best-effort revoking the refresh token server-side.
- **Also found and fixed while validating in the browser**: `frontend/Dockerfile` had no `.dockerignore`, so `COPY . .` baked the developer's local `.env.local` (`VITE_MOCK_API=true`, for `bun dev` against MSW mocks) into built Docker images — a real deployment risk, since the container would silently serve fabricated fixture data instead of hitting the real API. Added `frontend/.dockerignore`.
- Verification: `py_compile` on all changed backend files; `bun run build` clean; full end-to-end browser validation against a real (temporarily `DEV_MODE=true`, with the user's explicit sign-off since it seeds fixture data into the live DB) Docker stack — confirmed the import failed-record surface/edit/requeue/confirm round-trip, both split-visibility scenarios exactly as the user specified them (Jan-dated pending split visible through July; Jan+Feb bundle split visible in both months only after settling and only those two months), and zero new console errors. No local test DB was used for pytest since the only reachable Postgres in this session is the user's live database.
...
## Completed Tasks
- Diagnosed an "opening_balance already exists" report: the referenced account didn't exist in the production DB at all — a stale frontend-held account id, not a backend bug. No backend change made; confirmed via read-only `psql` queries against the user's own docker DB.
- Fixed a real timezone bug in the same edit flow: `TransactionForm`'s date field read/wrote via `initial.transacted_at.slice(0, 16)` — a raw UTC ISO string dropped into a `datetime-local` input, which has no timezone concept and treats the string as literal wall-clock time. For anyone not in UTC this showed the wrong time on open and, even if untouched, silently shifted the saved instant by the browser's offset on every save. Replaced with a `toDatetimeLocalValue(date: Date)` helper using local `Date` components.
- Fixed a stale-cache bug: `usePatchTransaction`/`useDeleteTransaction` (`api/transactions.ts`) invalidated only the plural `['transactions']` list key, never the singular `['transaction', id]` key — a different noun, so React Query's prefix matching never covered it. `SplitForm`, `SplitDrawer`, `SplitDetail`, and the transaction edit page all read that singular key directly and kept serving pre-edit data for up to 5 minutes. Added the missing invalidation to both mutations.
- Explore-agent audit confirmed no other `api/` file has the same mismatched-noun cache-key bug (everywhere else the singular detail key shares the plural key's prefix, so invalidation already cascaded).
- 3 new regression tests (timezone round-trip, both mutations' per-id cache invalidation) — DONE

# Ad-hoc Fix Sprint (2026-07-11, cont. 2) — Dashboard Period Balances + Pagination Move

## Completed Tasks
- Account Balances section (and the Total Balance hero stat) now show the balance as of the end of the selected dashboard period instead of the account's live current_balance; renamed `AccountBalanceItem.current_balance` → `.balance` end-to-end since the old name became misleading. Added an "Account Balances — as of {date}" label.
- New `_balance_delta_since` helper in `routers/dashboard.py` computes the net income/expense/transfer effect since a given instant (excluding opening_balance, which is a permanent seed); reused it to also de-duplicate `_cashflow_by_account`'s existing near-identical opening-balance calculation.
- Found and fixed a real, pervasive bug while doing this: transfer-in credit summed `Transaction.to_amount` directly, which is NULL for every same-currency transfer (the frontend never sends it) — silently dropping the credit for the destination account. The live-balance path already coalesced to `amount`; the dashboard's SQL now does too (`COALESCE(to_amount, amount)`). Since this fed a *running* balance in the cash flow chart, one dropped transfer permanently offset every later bucket — likely the cause of a user report that "the cash flow chart looks completely wrong."
...
- Follow-up fix: user immediately caught "Opening: 0.00" for a real account whose opening_balance transaction was dated exactly at the period's `from_date` — `compute_balances`'s `as_of` bound used strict `<` uniformly, silently dropping any opening_balance transaction that lands exactly on the boundary (the normal case for PDF-imported statements, always dated at midnight). Split the bound in `account_balance.py`: opening_balance rows use `<=`, income/expense keep `<` (avoids double-counting first-day transactions). Fixes the same bug in `dashboard.py`'s `_account_balances`/`_cashflow_by_account` for free, since they share the helper. New regression test; 56 tests pass.
- Second follow-up fix, deeper root cause: still wrong after the above — a Jan 1 (IST) transaction was leaking into the "Dec 2025" Transactions view. `Transactions.tsx`/`Disputes.tsx` built `from`/`to` by reattaching a literal `'T00:00:00.000Z'`/`'T23:59:59.999Z'` to a bare `YYYY-MM-DD` date string, silently treating the user's local calendar date as UTC — off by the timezone offset (5.5h for IST) at every period boundary. New `toLocalStartOfDayISO`/`toLocalEndOfDayISO` in `period.ts` let `.toISOString()` do the local→UTC conversion correctly from the already-correct local-time `Date` objects `resolvePeriod()` builds, instead of reconstructing a fake UTC string from stripped date components. Exposed as `rangeStart`/`rangeEnd` on `usePeriod()`; wired into the only two naive call sites. Confirmed via SQL that a related "still broken" report was actually a separate data-entry mistake (editing a datetime-local input's month/day without touching the year field, landing a transaction a full year off) — unwound by re-editing, unrelated to any app bug. Flagged, not fixed: the identical UTC-vs-local bug independently exists in `dashboard.py`'s custom-period query params and in Budgets/Splits pages' date handling — same root cause, wider blast radius, left for a dedicated follow-up. 2 new tests in `period.test.ts` (15 total pass); `bun run build` clean.

# Ad-hoc Fix Sprint (2026-07-11, cont. 5) — Timezone Consistency: dashboard.py, budgets.py, Splits Pages

## Completed Tasks
- Extended the UTC-vs-local fix to every remaining place with the same bug, per user's explicit ask: backend endpoints filtering by a period boundary against a real timestamp column now take a full `datetime` (never a bare `date` that forces a UTC assumption); the browser is the only place that ever converts to/from local time.
- `dashboard.py::home_dashboard`'s `start_date`/`end_date` changed to `datetime`; `_period_window`'s custom branch no longer reconstructs a UTC instant from date components — it trusts the caller's already-correct instant.
- `budgets.py`: `list_budgets` gained `spent_from`/`spent_to: datetime` for the transaction-spend and `activated_at` comparisons (kept `from_date: date` unchanged for the one genuinely date-only comparison, `Budget.end_date`); `list_budget_transactions`'s `from`/`to` changed to `datetime` outright.
- `Splits.tsx`/`SplitsAll.tsx`: client-side period filters were slicing a full UTC timestamp's first 10 characters directly (`s.expense_date.slice(0,10)`), extracting the UTC calendar date instead of the local one — fixed to convert to a local `Date` first via `toIsoDate(new Date(...))`.
- New `toLocalExclusiveEndISO` in `period.ts`; `usePeriod()` now exposes `rangeStart`/`rangeEnd` (inclusive)/`rangeEndExclusive` — wired into `Dashboard.tsx`, `Budgets.tsx`, `BudgetDrawer.tsx`, `BudgetDetail.tsx`.
- Found and fixed a distinct bug while unblocking `test_dashboard.py`: `_cashflow_by_account`'s per-bucket net-change query never included `opening_balance`-type transactions, so an account created mid-period had its opening balance silently vanish from the per-account cashflow chart (neither Step 1's "as of period_start" balance nor Step 2's flow sum ever counted it). Fixed by including `opening_balance` as a credit in Step 2, matching `account_balance.py`'s own treatment.
- Fixed the test-infra gap that was actively blocking verification: `db_tables` fixture used `Base.metadata.create_all`, which skips raw-SQL Alembic objects — `transaction_with_net_amount` (migration 0027) never existed in the test DB, failing all 23 dashboard-related tests regardless of any code change. Recreated the view by hand in the fixture. This unblocked 3 more pre-existing test bugs from last session's Phase 1 refactor (never caught since these tests couldn't run before) — fixed all three.
- Full backend suite: 486 passed, 8 failed (all pre-existing/unrelated: export/CLI/import-archive/payment-methods/reports-query). Full frontend suite: 466 passed, 64 failed (exactly the pre-existing ToastProvider/PeriodProvider gap, confirmed via `git stash` — zero new failures). `bun run build` clean.

# Ad-hoc Fix Sprint (2026-07-11, cont. 6) — Investigated and Fixed the 8 Remaining Test Failures

User asked whether the 8 "pre-existing" failures should actually be fixed rather than left flagged. Investigated each individually instead of assuming — found the data export/import feature was completely broken end to end in production, plus a real validation regression, plus genuinely stale tests.

## Completed Tasks
- **Export was completely broken**: `export_worker.py::_EXPORT_TABLES` referenced `llm_activity_logs` (plural) — the real table is `llm_activity_log` (singular, per its migration and model). Every export failed with `UndefinedTableError` before producing anything. Fixed, plus the same typo in both import paths' user_id-remap sets.
- **Import was completely broken, two separate ways**: (1) both `cli.py::_import_archive` and `routers/export.py::import_archive` called `session.begin()` after an earlier query on the same session had already auto-begun a transaction, raising `InvalidRequestError` before inserting anything; (2) neither path reversed export's UUID/Decimal/date/datetime → string flattening before re-inserting, so asyncpg rejected every timestamp/decimal/uuid column. Fixed both: removed the redundant `begin()`, added `deserialize_row()` (uses each column's real ORM type via `Base.metadata`, not a string-shape guess).
- **`user_settings` always collided on import**: it's a one-row-per-user table auto-created at signup, so importing the archived row for an existing target user always hit the primary key. Fixed by deleting the target's existing row first, in both import paths.
- **Restored an accidentally-deleted validation**: `PaymentMethodCreate` lost its "upi_app required when type is upi" check in an unrelated commit (`467bc3b`, hard-delete/confirmation-dialog work) — confirmed via `git show` this was collateral damage, not an intentional simplification. Restored it; UPI payment methods can no longer be created without specifying which app.
- **Updated 3 genuinely stale tests** to match current, correct (in one case *better*) behavior rather than an old spec: `reports/query`'s missing-user_id test expected rejection, but the actual design auto-injects the filter via AST rewrite (safer — can't be forgotten) — rewrote to assert the real, stronger property. Two export tests called `/auth/setup` twice, which this single-user-by-design app rejects on the second call — switched to the established `register_second_user` invite-flow helper.
- **Made Redis-dependent tests deterministic**: several tests assumed "Redis unreachable → falls back to inline execution," relying on the real environment lacking Redis rather than enforcing it — this sandbox has a live Redis, so the job actually enqueued and stayed "pending" forever. Added `_force_redis_unavailable()` (mocks `arq.create_pool` to raise), matching the pattern the file's own passing mock-based test already used.
- **The last one wasn't a bug**: `test_roundtrip_export_import` still 409'd with a UUID conflict after everything else — correctly, since the source account's id was still live in the same database. That's the conflict guard working as designed (a real safety feature against silent primary-key duplication); updated the test to delete the source data first, simulating the feature's actual intended use (migrating to a fresh install), not co-existing with it.
- Full backend suite: **494 passed, 0 failed, 7 skipped** — every test in the repository passes.

# Ad-hoc Fix Sprint (2026-07-12) — PDF Import Timezone Bug + Filter Persistence

- Found the actual cause of "Opening: 0.00" persisting for ALL accounts on the Year view: `imports.py::_record_to_transaction` hardcoded `tzinfo=UTC` when converting a bank statement's date-only field to a transaction timestamp. A calendar date like "01 Jan 2026" is meant in the user's local timezone, not UTC — for IST this landed the transaction 5.5h after the correct period boundary, pushing it to the wrong side of the `<=` comparison fixed the day before. Fixed using the user's own `UserSettings.timezone` (already stored, never previously read anywhere) via `ZoneInfo` — not a hardcoded assumption, the user's own configured value. New regression test in `test_imports.py`; 495 tests pass.
- Fixed transaction filters resetting when editing a transaction and returning: `TransactionFormPage` always navigated to a bare `/transactions` (no search params) on Back/Save, discarding whatever filters were active. Since that page is only ever entered from the Transactions list, switched to `router.history.back()`, which naturally restores the exact previous URL (filters, sort, pagination) with no state-threading needed.
