...
- [x] **M6** — Split creation is non-atomic two-step from form. Fixed: removed split creation from TransactionForm entirely. Splits can only be created by bundling existing transactions via BundleAsSplitModal → POST /splits/bundle (already atomic server-side). Deleted SplitSharesEditor.tsx and its test; removed useCreateSplit hook and SplitShareCreate/SplitCreate types from splits.ts.
