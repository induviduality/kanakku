...
**Decision:** Rather than adding a new abstraction (e.g. a wrapping mutation hook), extended the existing per-call pattern (`try/catch` + `toast(...)` for `mutateAsync`, `{ onSuccess, onError }` callbacks for `.mutate`) into every gap found, file by file: Accounts (account + payment method CRUD), Payees, Budgets (+ create/delete), AccountDrawer's status toggle, all PiggyBank surfaces (list, detail, drawer, create/edit form), Transactions (delete + inline description patch), Subscriptions (+ its create/edit form), the whole Splits surface (Splits/SplitsAll list delete, SplitDrawer/SplitDetail settle/forgive/edit/reset/delete, SplitForm create/update, inline payee creation), Reports/ReportDashboard (dashboard and widget create/delete, plus widget drag-layout save errors), and the remaining gaps in ImportReview. Added missing `ConfirmDialog`s for actions that previously fired immediately: SplitDrawer/SplitDetail's "unlink settlement" (×), ReportDashboard's widget delete and Reports' dashboard delete, ImportReview's bulk "Reject" and duplicate-resolution "Replace existing" (soft-deletes a matched transaction), and Disputes' "keep this one" duplicate-resolution card (previously relied on descriptive modal text alone with no secondary confirmation step).

**Kept deliberately unstyled/minimal:** no new toast variant for "success" — reused the existing `default` variant rather than adding a green/positive one, to avoid touching the toast component's visual design as part of a behavior-only sweep.

**Affects:** ~20 frontend files across `pages/`, `components/drawers/`, and `components/SplitForm.tsx`. No backend changes. `bun run build` clean, zero new TypeScript errors. Not browser-verified end-to-end given the sweep's breadth — worth a spot-check of a few flows (delete an account, reject import records, unlink a split settlement) before considering this fully verified.
