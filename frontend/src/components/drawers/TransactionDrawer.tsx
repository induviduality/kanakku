...
function useLookupMaps() {
  ...
  return { accountMap, payeeMap, categoryMap, tagMap }
}
...
export function TransactionDrawer({ transaction: txn, onClose }: Props) {
  const { accountMap, payeeMap, categoryMap, tagMap } = useLookupMaps()
      ...
      {txn && (
        <div className="space-y-6 p-5">
          ...
          <DrawerSection label="Details">
            <div className="kk-panel">
              ...
              {isTransfer && txn.to_amount && (
                <DrawerRow
                  label="To amount"
                  value={<span className="kk-mono">₹{parseFloat(txn.to_amount).toLocaleString('en-IN')}</span>}
                />
              )}
              {txn.description && (
          ...
          <DrawerSection label="Meta">
            <div className="kk-panel">
              <DrawerRow label="ID" value={<span className="kk-mono text-fg-faint">{txn.id}</span>} />
              <DrawerRow
                label="Created"
                value={new Date(txn.created_at).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}
              />
              {txn.subscription_id && <DrawerRow label="Subscription" value={txn.subscription_id.slice(0, 8) + '…'} />}
            </div>
          </DrawerSection>
        </div>
      )}
